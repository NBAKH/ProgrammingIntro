...
void serialEvent (Serial myPort) {
  ...
  if (inString != null) { //<>//
    String[] input = splitTokens(inString); //<>//
    print(input[0]);
    print("#");
    println(input[1]);
    // trim off any whitespace:
    input[0] = trim(input[0]);
    if(input[0].equals("q")==true){ //<>//
      // convert to an int and map to the screen height:
      float value = float(input[1]);  //<>//
      value = map(value, 0, 1023, 0, height-hPlayer2); //<>//
      yPlayer2=value; //<>//
    }
    if(input[0].equals("w")==true){
      // convert to an int and map to the screen height:
      float value = float(input[1]); 
      value = map(value, 0, 1023, 0, height-hPlayer1);
      yPlayer1=value;
    }
